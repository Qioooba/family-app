    ...
    <scroll-view class="reminder-list" scroll-y>
      ...
      >
        <view class="card-left">
          <view class="type-icon" :style="{ background: getIconBg(item.reminderType) }">
        ...
        <view class="card-body">
          <view class="card-header">
            ...
            <text class="reminder-type">{{ getTypeText(item.reminderType) }}</text>
          </view>
      ...
      <view v-if="reminders.length === 0 && !loading" class="empty-state">
        ...
        <text class="empty-title">暂无任务提醒</text>
        <text class="empty-desc">点击右上角+创建新提醒</text>
      </view>
    </scroll-view>
...
.card-tags {
  ...
  .tag-all { background: #e6f7ff; color: #1890ff; }
  .tag-specified { background: #f6ffed; color: #52c41a; }
  .tag-quiet { background: #fff7e6; color: #fa8c16; }
  .tag-workday { background: #f9f0ff; color: #722ed1; }
}
